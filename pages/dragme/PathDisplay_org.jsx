...
function openChildUrl(reDirectUrl) {
	confirmAlert({
		title: 'Open new window?',
		message: 'This will open a new browser window',
		buttons: [
			{
				label: 'Ok',
				// onClick: () => {},
				onClick: () => {redirectMe(reDirectUrl);}
			},
			{
				label: 'Stay here',
				onClick: () => {},
...
function redirectReview_v02(childId, emailAddress) {
	let redirectUrl = '';
	/*
	Call an action that returns new state for this instead of the call to apidev
	and redirect to invitation
	 */
...
function SquareOnly(props) {
	...
	if ((squareDetail !== undefined) &&
		...
		if (squareDetail[0].child_id) {
			child_id = squareDetail[0].child_id;
		}
		if (squareDetail[0].childUrl) {
			childUrl = squareDetail[0].childUrl;
		}
		if (squareDetail[0].square_description) {
			squareDescription = squareDetail[0].square_description;
		}
		if (squareDetail[0].square_name) {
			squareName = squareDetail[0].square_name;
		}
		if (squareDetail[0].image_path) {
			imagePath = squareDetail[0].image_path;
		}
		if (squareDetail[0].square_url) {
			squareUrl = squareDetail[0].square_url;
		}
		if (squareDetail[0].reserve_url) {
			reserveUrl = squareDetail[0].reserve_url;
		}
		if (squareDetail[0].video_url) {
			videoUrl = squareDetail[0].video_url;
		}
		if (squareDetail[0].map_url) {
			mapUrl = squareDetail[0].map_url;
		}
...
class PathDisplay_org extends Component {
	constructor(props) {
		super(props);
		this.state = {
...
export default withRouter(PathDisplay_org);